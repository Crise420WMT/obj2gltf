...
var facePattern2 = /f( +(-?\d+)\/(-?\d+)\/?)( +(-?\d+)\/(-?\d+)\/?)( +(-?\d+)\/(-?\d+)\/?)( +(-?\d+)\/(-?\d+)\/?)?( +(-?\d+)\/(-?\d+)\/?)*/;                            
...
function loadObj(objPath, options) {
    ...
    function addFace(vArr, pArr, uArr, nArr) {
        if (vArr.length > 3) {
            for (var i = 2;i < vArr.length; i++) {
                primitive.indices.push(addVertex(vArr[0], pArr[0], uArr[0], nArr[0]));
                primitive.indices.push(addVertex(vArr[i-1], pArr[i-1], uArr[i-1], nArr[i-1]));
                primitive.indices.push(addVertex(vArr[i], pArr[i], uArr[i], nArr[i]));
            }
        } else {
            // debugger
            primitive.indices.push(addVertex(vArr[0], pArr[0], uArr[0], nArr[0]));
            primitive.indices.push(addVertex(vArr[1], pArr[1], uArr[1], nArr[1]));
            primitive.indices.push(addVertex(vArr[2], pArr[2], uArr[2], nArr[2]));
        }
    }
    ...
    function parseLine(line) {
        ...
        if ((line.length === 0) || (line.charAt(0) === '#')) {
            ...
            var extraData = /^f\s+/.exec( line );
            var lineData = line.slice( extraData[ 0 ].length );
            var vuns = [];
            var vArr = [];
            var pArr = [];
            var uArr = [];
            ...
            // var splitData = lineData.split(/\s+/);

            for ( var i = 0; i < splitData.length; i ++ ) {
                vArr.push(splitData[i]);
                vuns = vuns.concat( splitData[i].split( '/' ) );
            }

            for ( var i = 0; i < vuns.length / 3; i ++ ) {
                pArr.push( vuns[ 3 * i ] );
                // uArr.push( vuns[ 3 * i + 1 ] );
            }
            addFace(vArr,pArr,[],[]);
            // addFace(
            ...
            for ( var i = 0; i < vuns.length / 2; i ++ ) {
                pArr.push( vuns[ 2 * i ] );
                uArr.push( vuns[ 2 * i + 1 ] );
            }
            addFace(vArr,pArr,uArr,[]);
            ...
            for ( var i = 0; i < vuns.length / 3; i ++ ) {
                pArr.push( vuns[ 3 * i ] );
                uArr.push( vuns[ 3 * i + 1] );
                nArr.push( vuns[ 3 * i + 2 ] );
            }
            addFace(vArr,pArr,uArr,nArr);
            // addFace(
